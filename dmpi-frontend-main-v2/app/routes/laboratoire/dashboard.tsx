...
import { getMesDemandesLaboratoire, type DemandeExamen } from "../../services/demandeExamenService";
      ...
      <Card accentBorder="border-t-4 border-[var(--color-primary)]">
        ...
        {loading ? (
          ...
          <ul className="flex flex-col gap-3">
            {enAttente.map((d) => (
              <li key={d.id} className="flex flex-wrap items-center justify-between gap-3 p-3 rounded-xl" style={{ backgroundColor: "var(--color-surface-container-low)" }}>
                <div>
                ...
                <Button size="sm" icon="upload" onClick={() => setDemandeAUploader(d)}>Déposer le résultat</Button>
              </li>
            ))}
          </ul>
        )}
